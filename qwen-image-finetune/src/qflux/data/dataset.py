...
class ImageDataset(Dataset):
    ...
    def _load_all_datasets(self):
        ...
        for dataset_path in self.dataset_paths:
            ...
            elif isinstance(dataset_path, str) and (dataset_path.endswith(".yaml") or dataset_path.endswith(".yml")):
                samples = self._load_yaml_dataset(dataset_path)
            else:
    ...
    def _load_yaml_dataset(self, dataset_path: str) -> list[dict]:
        ...
        try:
            ...
            logging.error(f"Failed to read yaml file {dataset_path}: {e}")
            return []
        ...
        # We need a counter for local_index.
        local_idx = 0

        for pair in pairs:
            # good image (target)
            good_rel = pair.get("good")
            if not good_rel:
                continue
            image_path = os.path.join(base_dir, good_rel)

            bads = pair.get("bads", [])
            for bad in bads:
                # bad image (control/input)
                bad_rel = bad.get("file")
                if not bad_rel:
                    continue
                control_path = os.path.join(base_dir, bad_rel)

                # prompt
                prompt = bad.get("enhancement", "")
                if not prompt:
                    # Fallback or empty prompt logic? Using empty string for now.
                    prompt = ""
                ...
                sample = {
                    ...
                    "caption": prompt,
                    "dataset_type": "local_yaml",
                    "local_index": local_idx,
                    "global_index": start_idx + local_idx,
                }

                # Check for mask if available? Not in current structure but handling if it appears
                if "mask" in bad:
                    sample["mask_file"] = os.path.join(base_dir, bad["mask"])
...
def collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ```
    """
    # [{a:1,b:2, c:{d:1,g:2}},{a:3,b:4, c:{d:3,g:4}}] -> {a: [1,3], b: [2,4], c:{d: [1,3], e: [2,4], g: [2,4]}}
